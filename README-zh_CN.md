...
- **智能标准输入模式**：当提示词包含特殊字符或超过长度限制时，自动通过标准输入传递提示词，支持 `force_stdin` 强制开关
...
- `timeout_secs`（数字）：codex 执行超时时间，单位为秒（默认：600，最大：3600）
- `force_stdin`（布尔值）：强制通过标准输入传递提示词，而非命令行参数（默认：`false`）。启用时，提示词始终通过标准输入管道传递，不受内容影响。当提示词超过 800 个字符或包含 Shell 特殊字符时，此功能也会自动触发。
